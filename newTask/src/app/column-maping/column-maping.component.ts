...
@Component({
  selector: 'app-column-maping',
  templateUrl: './column-maping.component.html',
  styleUrls: ['./column-maping.component.scss']
})
export class ColumnMapingComponent implements OnInit {
  ...
  getCheckedItemList() {
    this.checkedList = [];
    for (var i = 0; i < this.ruledata.length; i++) {
      if (this.ruledata[i].isSelected) this.checkedList.push(this.ruledata[i]);
    }
    console.log('this.checkedList', this.checkedList);
  }
  ...
  goToDQReports() {
    this.router.navigate(['/dataQualityCheck/dqReports']);
  }
